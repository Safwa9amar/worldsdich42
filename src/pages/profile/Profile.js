...
export default function Profile() {
  ...
  const { userData, isLogged, setIsLogged, isLoading } = useContext(Credentiel);

  useEffect(() => {
    setHeaderText(isLogged ? "Mon Compte" : "S'identifier");
  }, [isLogged]);
  return (
    <div className="flex flex-col items-center w-full">
      ...
      {!isLogged && isLoading && (
        <div role="status" className="p-20">
          <svg
            aria-hidden="true"
      ...
      <div className="mx-4 md:w-4/5">
        ...
        {!isLogged && (
          ...
          >
            <CredentielClient setHeaderText={setHeaderText} />
          </motion.div>
        )}
      </div>
    </div>
